...
def run_solver(strips, fleet, manifest, w, lm, time_limit_s: int = 30):
    """Run BOTH engines and keep whichever actually produces the better result,
    measured by the real objective (pc.plan_cost) — not by assuming either one
    is always better. OR-Tools generally wins on larger or looser problems, but
    measured directly against this app's typical manifests (many flight-tag-
    restricted demands, tight ±10min schedule windows, small mixed fleets),
    the heuristic has repeatedly and reproducibly beaten OR-Tools in head-to-
    head comparison. Guessing which one to trust up front was the wrong
    approach; comparing actual outcomes every time is the right one.

    Returns (routes, dropped, engine, note). `note` is a genuine OR-Tools
    failure reason (error / no solution) when OR-Tools couldn't run at all;
    it's None when both engines ran and the better one was picked fairly —
    the caller should only show a warning for the former, not the latter."""
    from demo_solver import solve as heuristic_solve
    routes_h, dropped_h = heuristic_solve(strips, fleet, manifest, w, lm)
    rr_h = {reg: res for reg, (_, _, res) in routes_h.items()}
    cost_h, _ = pc.plan_cost(rr_h, sum(d.pax for d in dropped_h), w)

    routes_o = dropped_o = cost_o = None
    or_tools_failure = None
    try:
        ...
        if result is not None:
            routes_o, dropped_o = result
            rr_o = {reg: res for reg, (_, _, res) in routes_o.items()}
            cost_o, _ = pc.plan_cost(rr_o, sum(d.pax for d in dropped_o), w)
        else:
            or_tools_failure = "OR-Tools found no solution within the time limit"
    except Exception as e:
        or_tools_failure = f"OR-Tools error: {e}"

    if cost_o is not None and cost_o <= cost_h:
        return routes_o, dropped_o, "OR-Tools", None
    return routes_h, dropped_h, "Heuristic", or_tools_failure
        ...
        with st.expander(
            ...
            if st.button(f"💾 Save aircraft for {selected_flight}", key=f"btn_save_ac_{selected_flight}"):
                ...
                for _, _row in _edited_ac.iterrows():
                    ...
                    # An unselected SelectboxColumn cell comes back as pandas
                    # NaN, not an empty string -- and NaN is truthy in Python,
                    # so `nan or ""` evaluates to nan, producing the literal
                    # string "nan" once stringified. Check pd.isna() first.
                    _raw_next = _row.get("next_route")
                    _next = "" if pd.isna(_raw_next) else str(_raw_next).strip()
...
with tab_optimize:
    ...
    if df_manifest is not None:
        ...
        if sr is not None:
            ...
            elif sr["engine"] == "Heuristic":
                st.info(
                    "Both engines were run and compared on this manifest — the heuristic "
                    "produced the better result (more pax served / lower cost), so it was kept."
                )
